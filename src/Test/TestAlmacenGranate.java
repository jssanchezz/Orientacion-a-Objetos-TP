...
public class TestAlmacenGranate {
	...
	public static void main(String[] args) {
		...
		try {
			System.out.println("--->Escenario 3.A: Se crea un cliente con datos validos[Juan] y otros con datos invalidos[Pedro]<---");
		...
		try {
			System.out.println("--->Escenario 4.A: Se intenta agregar un carrito nuevo a un cliente que tiene un carrito abierto<---");
		...
		try {
			System.out.println("--->Escenario 4.B: Crea un nuevo carrito para el cliente una vez que este cierra el que tiene<---");
			miComercio.agregarCarrito(LocalDate.now(), LocalTime.now(), juan);
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}
		...
		try {
			System.out.println("--->Escenario 5.C: Intentamos sacar unidades de un articulo que no existe en el carrito<---");
			miComercio.traerCarrito(2).sacar(miComercio.traerArticulo(1), 2);
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}
		...
		try {
			System.out.println("--->Escenario 6.A: Se realiza el cierre del carrito y se pasa a facturar el precio total<---");
			miComercio.traerCarrito(2).agregar(miComercio.traerArticulo(1), 3);
		...
		//Probamos el ticket
		try {
			System.out.println("--->Escenario 6.B: Se imprime el ticket total de la compra con los descuentos aplicados<---");
